    ...
    if (!empty($categorys)) {
      foreach ($categorys as $key => $r) {
        if (!$r['type']) {
          ...
          $r['add_icon'] = "<a target='right' href='". U('Content/index') ."?catid=".$r['id']."' onclick=javascript:openwinx('". U('Content/add') ."?catid=".$r['id']."','')><img src='". BASE_URL ."assets/images/admin/add_content.gif' alt='添加内容'></a> ";
        }
        $this->categorys[$r['id']] = $r;
      }
    }
