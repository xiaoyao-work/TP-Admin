...
class PositionController extends CommonController {
  ...
  public function edit() {
    if (IS_POST) {
      ...
      $types = $this->type_db->where("siteid = %d", $this->siteid)->select();
      $this->assign("types",$types);
      $this->assign('modelstr', \Org\Util\Form::select($model_array,$position['modelid'],'name="info[modelid]" onchange="category_load(this);"', '请选择模型'));
  ...
  public function public_category_load() {
    ...
    $category = \Org\Util\Form::select_category('','','name="info[catid]"','≡ 作为一级栏目 ≡',$modelid);
    echo $category;
  }
  ...
  public function public_model_load() {
    ...
    $modelstr = \Org\Util\Form::select(array_translate($models),'','name="info[modelid]" onchange="category_load(this);"','选择模型');
    echo $modelstr;
  }
