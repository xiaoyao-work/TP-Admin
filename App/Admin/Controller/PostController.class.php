...
class PostController extends CommonController {
    ...
    public function index() {
        ...
        if ($model['type'] == 2) {
            ...
            // 暂时对会员模型做一下兼容性处理
            if ($parentModel['id'] == 15) {
                $search_title_field = 'username';
            }
        ...
        // 获取文章
        $data = $post_logic->getPosts($fields['select_fields'], ['listorder' => 'desc', 'id' => 'desc'], 20);
                    ...
                    foreach ($taxonomies as $taxonomy) {
                        ...
                        if (!empty($tax_terms)) {
                            foreach ($tax_terms as $key => $value) {
                                $category_post_datas[] = ['term_id' => $value, 'post_id' => $post_id, 'post_type' => $taxonomy['post_type']];
                            }
                        }
                    }
    ...
    public function edit() {
        if (IS_POST) {
            ...
            if ($this->db->editContent($post_id, $data)) {
                ...
                // 分类处理结束
                $this->db->commit();
                $this->success('更新成功!', U('Post/index', array('moduleid' => $model['id'])));
            } else {
                $this->db->rollback();
                $this->error('更新失败！');
            }
    ...
    public function getPosts() {
        ...
        // 获取文章
        $data              = $post_logic->getPosts($fields['select_fields'], ['listorder' => 'desc', 'id' => 'desc'], 10);
