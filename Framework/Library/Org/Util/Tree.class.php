...
class Tree {
    ...
    *      )
    */
    public function init($arr=array(), $pid_str = 'parentid'){
        $this->arr = $arr;
        $this->ret = '';
    ...
    */
    ...
    public function get_taxonomy_tree($myid, $term_ids) {
        ...
        if(is_array($childs)){
            foreach($childs as $id => $value){
                ...
                $this->ret .= $nstr;
                $this->get_taxonomy_tree($value['id'], $term_ids);

                $nstr2 = $value['child'] ? '</ul></li>' : '</li>';
                $this->ret .= $nstr2;
            }
        }
        return $this->ret;
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        ...
        if(is_array($child)){
            ...
            foreach($child as $id=>$value){
                ...
                @extract($value);
                (${$this->pid_str} == 0 || $this->get_child( $id ) ) && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
    ...
    * 得到树型结构
    * @param int $myid，表示获得这个ID下的所有子级
    * @param string $str ul 前半部分，例如："<li><a href=\".U('Content/show?id='.\$id).\"> \$spacer \$catname</a>"
    * @param string $str ul 后半部分，例如："</li>"
    * @return string
    */

    public function get_tree_ul($myid, $str, $str2){
        $childs = $this->get_child($myid);
        if(is_array($childs)){
            foreach($childs as $id=>$value){
                @extract($value);
    ...
    public function get_menu_bootstrap($myid, $str, $str2){
        $childs = $this->get_child($myid);
        if(is_array($childs)){
            foreach($childs as $id=>$value){
                @extract($value);
                if($child) {
                    if ($parentid == 0 ) {
                        $menu_position = '<span class="caret"></span>';
                    } else {
                        $menu_position = '<span class="caret-right"></span>';
                    }
                    $nstr = '<li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="'. U('Content/lists?catid='.$id) .'">'. $catname . $menu_position .'</a><ul class="dropdown-menu">';
    // eval("\$nstr = \"$str\".\"<ul>\";");
                } else {
                    eval("\$nstr = \"$str\";");
                }
                $this->ret .= $nstr;
                $this->get_menu_bootstrap($id, $str, $str2);
                if($child) {
                    eval("\$nstr2 = \"</ul>\".\"$str2\";");
                } else {
                    eval("\$nstr2 = \"$str2\";");
